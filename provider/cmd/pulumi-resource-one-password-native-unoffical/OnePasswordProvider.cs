...
public class OnePasswordProvider : Provider
{
    ...
    public override async Task<ConfigureResponse> Configure(ConfigureRequest request, CancellationToken ct)
    {
        ...
        {
            Vault = news.Vault,
            ServiceAccountToken = news.ServiceAccountToken ?? Environment.GetEnvironmentVariable("OP_SERVICE_ACCOUNT_TOKEN"),
            ConnectHost = news.ConnectHost ?? Environment.GetEnvironmentVariable("OP_CONNECT_HOST"),
            ConnectToken = news.ConnectToken ?? Environment.GetEnvironmentVariable("OP_CONNECT_TOKEN"),
        };
    ...
    public override async Task<CheckResponse> CheckConfig(CheckRequest request, CancellationToken ct)
    {
        ...
        if (news.Vault is { })
        {
            outputs = outputs.Add("vault", new(news.Vault));
        }

        var pass = false;
        if (options is { ServiceAccountToken.Length: > 0 })
        {
            pass = true;
            outputs = outputs.Add("serviceAccountToken", new(options.ServiceAccountToken));
        }
        else
        {
            failures.Add(new CheckFailure("serviceAccountToken", "Service Account Token must be set"));
        }

        if (!pass && options is not { ConnectToken.Length: > 0, ConnectHost.Length: > 0 })
        {
            failures.Add(new CheckFailure("connectHost", "Connect Host must be set"));
            failures.Add(new CheckFailure("connectToken", "Connect Token must be set"));
        }
        else
        {
            outputs = outputs.Add("connectHost", new(options.ConnectHost!));
            outputs = outputs.Add("connectToken", new(options.ConnectToken!));
            pass = true;
        }
