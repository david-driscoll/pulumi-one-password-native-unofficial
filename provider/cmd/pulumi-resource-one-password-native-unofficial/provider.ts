...
export class Provider implements provider.Provider {
    ...
    constructor(readonly version: string, readonly schema: string) {
        ...
        this.config = new pulumi.Config("one-password-native-unofficial");
    }
    ...
    private diffValues(resourceType: ReturnType<typeof getResourceTypeFromUrn>, olds: Inputs, news: Inputs) {
        ...
        function handleReservedProperties(item: ReturnType<typeof diff>[number] & { path: (string | number)[] }) {
            ...
            if (resourceType == 'one-password-native-unofficial:index:Item' && item.path[0].toString() === 'category') return true;
    ...
    async invoke(token: string, inputs: any): Promise<provider.InvokeResult> {
        ...
        switch (functionType) {
            case 'one-password-native-unofficial:index:GetVault': return this.getVault(inputs);
            case 'one-password-native-unofficial:index:GetSecretReference': return this.getSecretReference(inputs);
            case 'one-password-native-unofficial:index:GetAttachment': return this.getAttachment(inputs);
