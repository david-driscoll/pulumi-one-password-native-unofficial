...
public class PropertyValueSerializer
{
    ...
    private string PropertyName(PropertyInfo property)
    {
        ...
        if (inputAttribute is InputAttribute input)
        {
            var name = (string)typeof(InputAttribute).GetProperty("Name", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(input);
    ...
    public async Task<PropertyValue> Serialize<T>(T value)
    {
        ...
        if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(InputMap<>) || targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(InputList<>))
        {
        ...
        //     targetType.GetInterfaces().FirstOrDefault(z => z.IsGenericType && z.GetGenericTypeDefinition() == typeof(IAsyncEnumerable<>)) is
        //         { } asyncEnumerableInterface)
        // {
        //     var inputType = asyncEnumerableInterface.GenericTypeArguments[0];
        //     return await (Task<PropertyValue>)typeof(PropertyValueSerializer)
        //         .GetMethod(nameof(ToArrayAsync), BindingFlags.NonPublic | BindingFlags.Instance)!
        ...
        if (value is IEnumerable enumerable)
        {
            ...
            if (value is IList list && InitializedByDefault(list))
            {
        ...
        if (targetType.GetCustomAttribute<EnumTypeAttribute>() is not null)
        {
        ...
        {
            var task = (Task<ImmutableDictionary<string, object?>>)
                typeof(InputArgs).GetMethod("ToDictionaryAsync", BindingFlags.Instance | BindingFlags.NonPublic)!.Invoke(inputArgs, Array.Empty<object>());
            var inputsMap = await task!.ConfigureAwait(false);
        ...
        {
            var propertyObject = ImmutableDictionary.CreateBuilder<string, PropertyValue>();
            foreach (var property in targetType.GetProperties().OrderBy(z => z.Name))
            {
                var propertyName = PropertyName(property);
    ...
    public object? DeserializeValue(PropertyValue value, Type targetType, string[] path)
    {
        ...
        {
            var elementType = targetType.GetGenericArguments()[0];
            var outputDataType = typeof(Input<>).Assembly.GetType("Pulumi.Serialization.OutputData`1")!.MakeGenericType(elementType);
